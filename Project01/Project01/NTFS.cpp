...
void NTFS::readBoot_Sector()
{
	...
	cout << "1. So byte cua sector: " << bytes_per_sector << " bytes" << endl;

	sectors_per_cluster = hexstr_to_int(to_hexstr(BootSector, 13, 0, 1, 1));
	cout << "2. So sector tren cluster: " << sectors_per_cluster << " sectors" << endl;

	cout << "3. Media descriptor: " << to_hexstr(BootSector, 5, 1, 1, 1) << " hard disk" << endl;

	cout << "4. Tong so sector NTFS: " << hexstr_to_int(to_hexstr(BootSector, 8, 2, 8, 1)) << " sectors" << endl;

	begin_MFT = hexstr_to_int(to_hexstr(BootSector, 0, 3, 8, 1));
	cout << "5. Cluster bat dau cua MFT: " << begin_MFT << endl;

	cout << "6. MFT mirror bat dau tai: " << hexstr_to_int(to_hexstr(BootSector, 8, 3, 8, 1)) << endl;
}
